...
function mountScrollWorld(container, config) {
  ...
  if (config.brand) {
    ...
    brand.setAttribute('aria-label', `${config.brand.name || 'Portfolio'} — back to top`);
    brand.appendChild(el('span', 'sw-brand__mark'));
    const nm = el('span', 'sw-brand__name'); nm.textContent = config.brand.name || ''; brand.appendChild(nm);
    topbar.appendChild(brand);
  }
  const nav = el('nav', 'sw-nav'); nav.setAttribute('aria-label', 'Portfolio chapters');
  ...
  const route = el('nav', 'sw-route'); route.setAttribute('aria-label', 'Portfolio chapter navigation');
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      c.toggleAttribute('inert', !interactive);
      if (interactive) c.removeAttribute('aria-hidden');
    ...
    if (near !== activeIndex) {
      ...
      nav.querySelectorAll('.sw-nav__item').forEach((n, k) => {
        ...
        if (k === near) n.setAttribute('aria-current', 'step');
  ...
  function ctaBtns(cta) {
    let h = '';
    const attrs = link => /^https?:/.test(link.href || '') ? ' target="_blank" rel="noopener noreferrer"' : '';
    if (cta.primary) h += `<a class="sw-btn sw-btn--primary" href="${esc(cta.primary.href || '#')}"${attrs(cta.primary)}>${esc(cta.primary.label)}</a>`;
    if (cta.secondary) h += `<a class="sw-btn sw-btn--ghost" href="${esc(cta.secondary.href || '#')}"${attrs(cta.secondary)}>${esc(cta.secondary.label)}</a>`;
    return h;
  }
}
